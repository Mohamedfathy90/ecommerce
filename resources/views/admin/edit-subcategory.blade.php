...
<div class="col-lg-10">
	<div class="card">
		...
		<form id="myForm" method="post" action="/edit-subcategory/{{$subcategory->id}}" enctype="multipart/form-data" >
			@csrf
			@method('patch')
            ...
            <div class="row mb-3">
			    ...
			    @foreach($categories as $category)
		 	    <option value="{{$category->id }}"  {{($category->name === $subcategory->category->name) ? 'selected' : '' }}>
                 {{ $category->name }} 
                </option>
		 	    @endforeach
